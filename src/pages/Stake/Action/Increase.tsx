...
const Increase = memo<TProps>(
  ({ intl, stakingConfig, fromMasterChef, wtfRewardsBalance, claimReward, rewardPerBlock, totalVeWTF }) => {
    ...
    const receivedVeWTF = useMemo(() => {
      ...
      if (fromMasterChef) {
        ...
        if (!locked) {
          ...
          return numeral((Number(_wtfRewardsBalance) * duration * multiplier) / 100 / MAX_LOCK_TIME).format(
            "0,0.[0000]"
          );
        }
        if (locked) {
    ...
    const convertRatio = useMemo(() => {
      ...
      console.log("convertratio", receivedVeWTF);
      if (!receivedVeWTF) return;
      ...
      if (balanceInput !== "0") {
        ...
        if (locked) {
          ...
          return numeral(Number(_receivedVeWTF) / Number(lockingWTF)).format("0,0.[0000]");
        } else {
